...
#define CNV_TEST_BASE(DTYPE_I, DTYPE_O) \
    ...
    int res_m = m - k_m + 1; \
    int res_n = n - k_n + 1; \
    DTYPE_O res_sw[res_m * res_n]; \
    DTYPE_O res_hw[res_m * res_n]; \
    init_array_##DTYPE_I(a, n, m); \
    init_array_##DTYPE_I(b, k_n, k_m); \
    MA_DEFINE_##DTYPE_I(0, m, n); \
    ...
    MA_STORE_REGISTER(2, res_hw); \
    printf(","); \
    cnv_##DTYPE_O(a, b, res_sw, m, n, k_m, k_n); \
    printf(","); \
    debug_cnv_##DTYPE_I(a, b, res_sw, res_hw, m, n, k_m, k_n, k_n); \
    return cmp_cnv_##DTYPE_O(res_hw, res_sw, res_n, res_m);
